class Sunny
  ...
  BOT.command :dehive, description: 'Delete all channels on the Archives category.' do |event|
    ...
    return 'You cannot do this action while inside the archives.' if event.channel.parent&.id == Setting.archive_category_id

    BOT.channel(Setting.archive_category_id).children.each(&:delete)
