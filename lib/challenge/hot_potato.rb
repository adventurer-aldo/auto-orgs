class Sunny
  ...
  BOT.command :pass do |event, *args|
    ...
    if event.message.mentions.size.positive?
      ...
      ids.each do |id|
        if Player.where(status: ALIVE, user_id: id, season_id: Setting.season_id).exists?
  ...
  def self.explode_potato
    ...
    if participants.size < 2
      channel.start_typing
      sleep(2)
      channel.send_message("I'm excited to announce that as the sole remaining castaway... **#{player.name.upcase} WINS INDIVIDUAL IMMUNITY!!**")
    end
  end
