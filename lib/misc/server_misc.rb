class Sunny
    BOT.member_join do |event|
        if event.server.id == SERVER_ID
            BOT.send_message(USER_JOIN_CHANNEL, sprintf(Setting.last.join_msg, event.user.name))
    ...
    BOT.member_leave do |event|
        if event.server.id == SERVER_ID
            BOT.send_message(USER_JOIN_CHANNEL, sprintf(Setting.last.leave_msg, event.user.name))
        end
    end
